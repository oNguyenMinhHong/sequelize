  ...
  run(sql, parameters) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (parameters) {
        ...
        let i = 0;
        parameters = parameters.map(param => typeof param === 'string' ? `'${param}'` : param);
        sql = sql.replace(/\?/g, () => parameters[i++]);
        // this.connection.execute(sql, parameters, handler).setMaxListeners(100);
        this.connection.query(sql).exec(handler);
      } else {
        this.connection.query(this.sql).exec(handler);
      }
    })
